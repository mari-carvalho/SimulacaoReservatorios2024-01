...
def plot_animation_map_2d(grid: dict):
    ...
    def update(frame):
        if frame == 0:
            ...
            plt.title("Distribuição de Pressão - Poço no Canto Superior Esquerdo")
            plt.tight_layout()
    ...
    ani.save(f'animacao_map_poco_esquerda_acima.gif', writer='pillow', fps=60)  # 1 frame por segundo
    plt.close()
...
plt.xlabel('X [m]')
plt.ylabel('Y [m]')
plt.title('Distribuição de Permeabilidade - Poço no Canto Superior Esquerdo')
plt.show()
