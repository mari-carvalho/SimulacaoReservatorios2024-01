...
def plot_animation_map_2d(grid: dict):
    ...
    def update(frame):
        if frame == 0:
            ...
            plt.title("Distribuição de Pressão - Poço no Canto Inferior Direito")
            plt.tight_layout()
    ...
    ani.save(f'animacao_map_poco_direita_abaixo.gif', writer='pillow', fps=60)  # 1 frame por segundo
    plt.close()
    ...
    for i in range(len(perm)):
        for j in range(len(perm)):
            ...
            if j != 0 and j != px and i != 0 and i != px:
                ...
                if i == poco +5 and j == poco +5: # poço a direita abaixo
                    req = 0.5612 * dx
...
plt.xlabel('X [m]')
plt.ylabel('Y [m]')
plt.title('Distribuição de Permeabilidade - Poço no Canto Inferior Direito')
plt.show()
...
plt.title('Vazão com Poço no Canto Inferior Direito')
plt.legend()
plt.xlabel('Tempo [h]')
...
plt.title('Produção Acumulada com Poço no Canto Inferior Direito')
plt.legend()
plt.xlabel('Tempo [h]')
