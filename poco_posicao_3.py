# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 10:01:03 2024
...
def plot_animation_map_2d(grid: dict):
    ...
    def update(frame):
        if frame == 0:
            ...
            plt.title("Distribuição de Pressão - Poço no Canto Inferior Esquerdo")
            plt.tight_layout()
    ...
    ani.save(f'animacao_map_poco_esquerda_abaixo.gif', writer='pillow', fps=60)  # 1 frame por segundo
    plt.close()
    ...
    for i in range(len(perm)):
        for j in range(len(perm)):
            ...
            if j != 0 and j != px and i != 0 and i != px:
                ...
                if i == poco + 5 and j == poco - 5: # poço a esquerda abaixo 
                    req = 0.5612 * dx
...
plt.xlabel('X [m]')
plt.ylabel('Y [m]')
plt.title('Distribuição de Permeabilidade - Poço no Canto Inferior Esquerdo')
plt.show()
...
plt.xlabel('X [m]')
plt.ylabel('Y [m]')
plt.title(f'Distribuição de Pressão em {tempo_list_horas[0]} segundos')
plt.show()
'''
...
tempo_smooth = np.linspace(min(tempo_list_horas), max(tempo_list_horas), 10000)
...
plt.title('Vazão com Poço no Canto Inferior Esquerdo')
plt.legend()
plt.xlabel('Tempo [h]')
...
plt.title('Produção Acumulada com Poço no Canto Inferior Esquerdo')
plt.legend()
plt.xlabel('Tempo [h]')
