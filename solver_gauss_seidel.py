...
def gauss_seidel(A, b, x0, Eppara, maxit):
    n = len(b) # medir o tamanho do vetor de P no tempo anterior mais 8/3*eta*rx*Pw(ou P0), número de equações do sistema
    x = np.zeros(n) if x0 is None else np.array(x0) # vetor de x com zeros se não for dado chute inicial, e vetor x0 se for fornecido chute inicial 

    iter = 0 
    Epest = np.linspace(100,100,n) # vetor de erros relativos
    ...
    while np.max(Epest) >= Eppara and iter <= maxit: # enquanto o valor máximo do vetor de erros for maior ou igual ao critério de parada e o número de interações for menor que o máximo
        ...
        for i in range(n): # loop nas equações do sistema 
